...
int find_name(Bodies bodies, uint32_t name)
{
    for (int i = 0; i < bodies.num; i++)
    {
        if (bodies.name[i] == name) return i;
    }
    return -1;
}

Bodies construct(const char **input, int input_size)
{
    BodyLink *links = (BodyLink*)malloc(sizeof(BodyLink)*input_size);

    for (int i = 0; i < input_size; i++)
    {
        const char *p = input[i];
        uint32_t parent_body_name = 0;
        while (p[0] != ')')
        {
            parent_body_name += p[0];
            parent_body_name <<= 8;
            p++;
        }
        p++; // skip )
        uint32_t body_name = 0;
        while (p[0])
        {
            body_name += p[0];
            body_name <<= 8;
    ...
    Bodies bodies = {
        .num = input_size,
        .name = (uint32_t*)malloc(sizeof(uint32_t)*input_size),
        .parent_index = (int*)malloc(sizeof(int)*input_size),
    };
    for (int i = 0; i < input_size; i++)
    {
        bodies.name[i] = links[i].name;
        bodies.parent_index[i] = -1;
    }

    for (int i = 0; i < input_size; i++)
    {
        bodies.parent_index[i] = find_name(bodies, links[i].parent_name);
    }
...
int orbits(Bodies bodies, int body)
{
    int parent = bodies.parent_index[body];
    int result = 0;
    while (parent != -1)
    {
        result++;
        parent = bodies.parent_index[parent];
    }
    return result + 1;
}

int total_orbits(Bodies bodies)
{
    int tot = 0;
    for (int i = 0; i < bodies.num; i++)
    {
...
void test()
{
    ...
    int input_size = (int)(sizeof(test_input)/sizeof(const char*));

    Bodies bodies = construct(test_input, input_size);
    int tot = total_orbits(bodies);
    printf("Total orbits %d\n", tot);
}

void part_one(Bodies bodies)
{
    int tot = total_orbits(bodies);
    printf("Total orbits %d\n", tot);
}
...
int orbit_transfers_between(Bodies bodies, int a, int b)
{
    int a_transfers = 0;
    int a_node = a;
    while (a_node != -1)
    {
        a_node = bodies.parent_index[a_node];
        int b_transfers = 0;
        int b_node = b;
        while (b_node != -1)
        {
            b_node = bodies.parent_index[b_node];
            if (a_node == b_node)
            {
                return a_transfers + b_transfers;
            }
            b_transfers++;
        }
        a_transfers++;
...
void part_two(Bodies bodies)
{
    uint32_t YOU = to_name("YOU");
    uint32_t SAN = to_name("SAN");
    int you = find_name(bodies, YOU);
    int san = find_name(bodies, SAN);

    int transfers = orbit_transfers_between(bodies, you, san);
    printf("%d\n", transfers);
}
...
void test_two()
{
    ...
    int input_size = (int)(sizeof(input)/sizeof(const char*));
    Bodies bodies = construct(input, input_size);
    part_two(bodies);
}
